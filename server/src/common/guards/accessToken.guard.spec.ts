...
describe('AccessTokenGuard (unit)', () => {
  ...
  const makeContext = (headers, reqObj = {}) => ({
    switchToHttp: () => ({
  ...
  it('throws when header malformed', async () => {
    await expect(guard.canActivate(makeContext({ authorization: 'Bad token' }))).rejects.toThrow(UnauthorizedException);
  });
  ...
  it('returns true and sets req.userId when ok', async () => {
    mockJwt.verifyAsync.mockResolvedValueOnce({ userId: 'u1' });
    const reqObj = { headers: { authorization: 'Bearer token' } };
