...
describe("AiService", () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        AiService,
        { provide: getRepositoryToken(ai_interview), useValue: mockAiInterviewRepo },
        { provide: getRepositoryToken(ai_transcript), useValue: mockAiTranscriptRepo },
  ...
  describe("createInterview", () => {
    it("creates an interview and returns entrypoint on success", async () => {
      ...
      mockHttpService.axiosRef.post.mockResolvedValueOnce({ status: 200, data: { data: "entry" } });
      mockAiInterviewRepo.save.mockResolvedValueOnce({ interview_id: "new-id" });

      const res = await service.createInterview({ type: "Technical" } as any, "user-1");
    ...
    it("throws ConflictException when interview already exists", async () => {
      mockAiInterviewRepo.findOne.mockResolvedValueOnce({ interview_id: "exists" });

      await expect(service.createInterview({} as any, "user-1")).rejects.toThrow(ConflictException);
    });

    it("throws InternalServerErrorException when AI server returns non-200", async () => {
      mockAiInterviewRepo.findOne.mockResolvedValueOnce(null);
      mockHttpService.axiosRef.post.mockResolvedValueOnce({ status: 500, data: {} });

      await expect(service.createInterview({} as any, "user-1")).rejects.toThrow(InternalServerErrorException);
    });
  ...
  describe("editAiInterview", () => {
    it("throws UnauthorizedException when interview not found", async () => {
      jest.spyOn(service, 'getInterviewById').mockResolvedValueOnce(null as any);

      await expect(service.editAiInterview('nope','' as any,'user-1')).rejects.toThrow(UnauthorizedException);
    });

    it("updates interview and returns true when successful", async () => {
      const existing = { ...mockInterview, status: AiInterviewStatus.ASKED } as any;
      jest.spyOn(service, 'getInterviewById').mockResolvedValueOnce(existing);
      mockAiInterviewRepo.save.mockResolvedValueOnce(true);

      const res = await service.editAiInterview('i1', { status: AiInterviewStatus.COMPLETED } as any, 'user-1');
    ...
    it('throws InternalServerErrorException when save fails on edit', async () => {
      ...
      await expect(service.editAiInterview('i1', { status: AiInterviewStatus.COMPLETED } as any, 'user-1')).rejects.toThrow(InternalServerErrorException);
    });
  });
  ...
  describe("getUserInterviews", () => {
    ...
    it('throws InternalServerErrorException when find throws', async () => {
      ...
      await expect(service.getUserInterviews({} as any, 'user-1')).rejects.toThrow(InternalServerErrorException);
    });

    it("returns paginated interviews when page/limit provided", async () => {
      mockAiInterviewRepo.findAndCount.mockResolvedValueOnce([[mockInterview], 10]);

      const res = await service.getUserInterviews({ page: 2, limit: 5 } as any, 'user-1');

      expect(mockAiInterviewRepo.findAndCount).toHaveBeenCalled();
      expect(res).toEqual({ data: [mockInterview], meta: { total: 10, page: 2, limit: 5 } });
    });

    it('throws InternalServerErrorException when findAndCount throws', async () => {
      mockAiInterviewRepo.findAndCount.mockRejectedValueOnce(new Error('counterr'));
      await expect(service.getUserInterviews({ page: 1, limit: 10 } as any, 'user-1')).rejects.toThrow(InternalServerErrorException);
    });
  });
  ...
  describe("addTranscripts", () => {
    it("throws UnauthorizedException when interview not found", async () => {
      ...
      await expect(service.addTranscripts('i1', { transcripts: [] } as any, 'user-1')).rejects.toThrow(UnauthorizedException);
    });

    it("saves transcripts and returns inserted count", async () => {
      jest.spyOn(service, 'getInterviewById').mockResolvedValueOnce({ interview_id: 'i1' } as any);
      mockAiTranscriptRepo.save.mockResolvedValueOnce([{ id: 1 }]);

      const res = await service.addTranscripts('i1', { transcripts: [{ content: 'hi', who_stated: 'user' }] } as any, 'user-1');
    ...
    it('throws InternalServerErrorException when transcript save fails', async () => {
      ...
      await expect(service.addTranscripts('i1', { transcripts: [{ content: 'hi', who_stated: 'user' }] } as any, 'user-1')).rejects.toThrow(InternalServerErrorException);
    });
  });
  ...
  describe('getInterviewById', () => {
    ...
    it('returns interview with transcripts when requested', async () => {
      mockAiInterviewRepo.findOne.mockResolvedValueOnce({ interview_id: 'i1', user_id: 'user-1' });
      mockAiTranscriptRepo.find.mockResolvedValueOnce([{ id: 1, content: 't1' }]);

      const res = await service.getInterviewById('i1', 'user-1', true);
      expect(mockAiInterviewRepo.findOne).toHaveBeenCalled();
      expect(mockAiTranscriptRepo.find).toHaveBeenCalled();
      expect(res).toEqual({ interview_id: 'i1', user_id: 'user-1', transcripts: [{ id: 1, content: 't1' }] });
    });
    ...
    it('throws InternalServerErrorException when repo errors', async () => {
      ...
      await expect(service.getInterviewById('i1', 'user-1')).rejects.toThrow(InternalServerErrorException);
    });
  });
});
